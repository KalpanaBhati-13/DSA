class Solution {
 public:
  int maxCollectedFruits(vector<vector<int>>& fruits) {
    int topLeft = getTopLeft(fruits);
    int topRight = getTopRight(fruits);
    int bottomLeft = getBottomLeft(fruits);

    // Subtract the bottom-right cell (n-1,n-1) twice to avoid overcounting
    int overlap = 2 * fruits.back().back();

    return topLeft + topRight + bottomLeft - overlap;
  }
